...
for snr_idx in range(snr_num):
    ...
    for exper_idx in range(exper_num):
        ...
        for epoch in range(n_epochs):
            ...
            del hab1, hba1, hab2, hba2, nb11, na12, nb23, na24, na21, nb22, na13, nb14, ha, hb, na, nb
        ...
        # MIs_arr[snr_idx][exper_idx] = max(MIs)


# Plotting
MIs_arr_mean = np.mean(MIs_arr,axis=1)

MIs_arr_mean_bit = np.log2(np.exp(1))*MIs_arr_mean
mi_numeri_bit = np.log2(np.exp(1))*mi_numeri

print("MINE estimate", MIs_arr_mean_bit)
print("Gaussian estimate", mi_numeri_bit)
...
fig, ax = plt.subplots()
ax.plot(range(len(MIs_arr_mean)), MIs_arr_mean, label='MINE estimate')
ax.plot(range(len(mi_numeri)), mi_numeri, label='Gaussian estimate')
