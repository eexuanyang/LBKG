...
max_arr = [1.02 1.03 1.04 1.05 1.06];
min_arr = [0.98 0.97 0.96 0.95 0.94];

for max_arr_idx = 1:length(max_arr)
    max_val = max_arr(max_arr_idx);
    min_val = min_arr(max_arr_idx);
    ...
    for seed_z = seed_z_arr
        ...
        k_val = (max_val-min_val)/(mid_temp_max - mid_temp_min);
        b_val = min_val-k_val*mid_temp_min;
    
        sig_z_csi_norm(:,seed_z) = (k_val*mid_temp + b_val).';
    end
    save(['sig_z_csi_norm' num2str(max_arr_idx) '.mat'],'sig_z_csi_norm')
end

seg_num = 100;
mv_len = 200;
range_arr = max_arr - min_arr;
for range_idx = 1:length(range_arr)
    load(['sig_z_csi_norm' num2str(range_idx) '.mat'])
    key_fdd_norm_sort_A_E_kdr_zz = zeros(1,length(seed_z_arr));
    key_fdd_norm_sort_A_E_kgr_zz = zeros(1,length(seed_z_arr));
    for seed_z = seed_z_arr
        key_arr_a = [];
        key_arr_e = [];
        ...
        key_fdd_norm_sort_A_E_kdr_zz(seed_z) = 1 - length(find((key_arr_fdd_a_norm_sort_ds_bit_disc_final == key_arr_fdd_e_norm_sort_ds_bit_disc_final)))/length(key_arr_fdd_a_norm_sort_ds_bit_disc_final);
        key_fdd_norm_sort_A_E_kgr_zz(seed_z) = 2*length(find((key_arr_fdd_a_norm_sort_ds_bit_disc_final == key_arr_fdd_e_norm_sort_ds_bit_disc_final)))/length(key_arr_fdd_a_norm_sort_ds_bit_disc_final);
        
    end
 
    save(['key_fdd_norm_sort_A_E_kdr_zz_range' num2str(range_idx) '.mat'],'key_fdd_norm_sort_A_E_kdr_zz')
    save(['key_fdd_norm_sort_A_E_kgr_zz_range' num2str(range_idx) '.mat'],'key_fdd_norm_sort_A_E_kgr_zz')
    toc
end
