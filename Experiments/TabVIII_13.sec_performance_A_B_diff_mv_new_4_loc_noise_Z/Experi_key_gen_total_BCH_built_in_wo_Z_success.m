...
% sig_z_csi_norm = zeros(1200, length(seed_z_arr));
% for seed_z = seed_z_arr
%     rng((seed_z-1)*5+1)
%     release(tdlzz)
%     tdlzz.Seed = double(rng().Seed);
%     sig_z = tdlzz(txWaveform);
%     sig_z_csi_1 = CSI_get_CFO_exist(sig_z, subframe_len, CP_array, fft_len, fs_aim);
%     rng((seed_z-1)*5+2)
%     release(tdlzz)
%     tdlzz.Seed = double(rng().Seed);
%     sig_z = tdlzz(txWaveform);
%     sig_z_csi_2 = CSI_get_CFO_exist(sig_z, subframe_len, CP_array, fft_len, fs_aim);
%     rng((seed_z-1)*5+3) 
%     release(tdlzz)
%     tdlzz.Seed = double(rng().Seed);
%     sig_z = tdlzz(txWaveform);
%     sig_z_csi_3 = CSI_get_CFO_exist(sig_z, subframe_len, CP_array, fft_len, fs_aim);
%     rng((seed_z-1)*5+4)
%     release(tdlzz)
%     tdlzz.Seed = double(rng().Seed);
%     sig_z = tdlzz(txWaveform);
%     sig_z_csi_4 = CSI_get_CFO_exist(sig_z, subframe_len, CP_array, fft_len, fs_aim);
%     rng((seed_z-1)*5+5)
%     release(tdlzz)
%     tdlzz.Seed = double(rng().Seed);
%     sig_z = tdlzz(txWaveform);
%     sig_z_csi_5 = CSI_get_CFO_exist(sig_z, subframe_len, CP_array, fft_len, fs_aim);
%     
%     sig_z_csi = sig_z_csi_1 + sig_z_csi_2 + sig_z_csi_3 + sig_z_csi_4 + sig_z_csi_5;
%     
%     mid_temp = abs(sig_z_csi(start_subcar:end_subcar));
%     mid_temp_min = min(mid_temp);
%     mid_temp_max = max(mid_temp);
%     k_val = (1.4-0.6)/(mid_temp_max - mid_temp_min);
%     b_val = 0.6-k_val*mid_temp_min;
%     sig_z_csi_norm(:,seed_z) = (k_val*mid_temp + b_val).';
% end
% % 
% % save('sig_z_csi_norm.mat','sig_z_csi_norm');
% load('sig_z_csi_norm.mat')

% seg_arr = [3 5 10 25 50 100];
seg_arr = 50;
% seg_arr = 5;
for seg_idx = 1:length(seg_arr)
    seg_num = seg_arr(seg_idx);
    key_fdd_norm_sort_kdr_wo_zz = zeros(1,length(seed_z_arr));
    key_fdd_norm_sort_kgr_wo_zz = zeros(1,length(seed_z_arr));
    for seed_z = seed_z_arr
        key_arr_a = [];
        key_arr_b = [];
        for data_set = data_set_arr
            if data_set == "230421"
                loc_arr = [2:9 11:17 19 20];
            ...
            for dev = dev_arr
                for mat_index = mat_arr
                    ...
                    sample_idx = 1;
                    for loc = loc_arr
        %                 load(['I:\Project\Key\' num2str(str2num(data_set)) '\Proceed_li\user' num2str(dev) '\data' num2str(loc) '\li_wo_CFO_rm_proceed_'  num2str(mat_index)  '.mat']);
                        load(['I:\Project\Key\' num2str(str2num(data_set)) '\proceed\user' num2str(dev) '\data' num2str(loc) '\proceed_'  num2str(mat_index)  '.mat']);
...
%                         figure
%                         plot(DMRS_arr_smooth,'r')
%                         hold on
%                         plot(DMRS_arr_proc,'b')
...
% %                             hold on
        ...
        key_fdd_norm_sort_kdr_wo_zz(seed_z) = 1 - length(find((key_arr_fdd_a_norm_sort_ds_bit_disc_final == key_arr_fdd_b_norm_sort_ds_bit_disc_final)))/length(key_arr_fdd_a_norm_sort_ds_bit_disc_final);
        key_fdd_norm_sort_kgr_wo_zz(seed_z) = 2*length(find((key_arr_fdd_a_norm_sort_ds_bit_disc_final == key_arr_fdd_b_norm_sort_ds_bit_disc_final)))/length(key_arr_fdd_a_norm_sort_ds_bit_disc_final);
    
        dlmwrite(['key_arr_fdd_wo_zz_seg' num2str(seg_num) '_seed' num2str(seed_z) '.out'], key_arr_fdd_a_norm_sort_ds_bit_disc_final, ' ');
    
    end

    save(['key_fdd_norm_sort_kdr_wo_zz_seg' num2str(seg_num) '.mat'],'key_fdd_norm_sort_kdr_wo_zz')
    save(['key_fdd_norm_sort_kgr_wo_zz_seg' num2str(seg_num) '.mat'],'key_fdd_norm_sort_kgr_wo_zz')
